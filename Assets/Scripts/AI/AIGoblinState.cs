...
// -----------------------------------------------------------------------------------------
// CLASS	:	AIGoblinState
// DESC		:	The immediate base class of all goblin states. It provides the event
//				processing and storage of the current threats.
// ----------------------------------------------------------------------------------------
public abstract class AIGoblinState : AIState
{
	// Private
	protected int _playerLayerMask = -1;
	protected int _bodyPartLayer = -1;
	protected int _visualLayerMask = -1;
	protected AIGoblinStateMachine _goblinStateMachine = null;
	private	  Collider _batCollider;
	...
	void Awake()
	{
		...
		_batCollider		= GameObject.Find("GoblinBat").GetComponent<SphereCollider>();
		_shieldCollider		= GameObject.Find("GoblinShield").GetComponent<BoxCollider>();
	}
	...
	{
		if (stateMachine.GetType() == typeof(AIGoblinStateMachine))
		{
			base.SetStateMachine(stateMachine);
			_goblinStateMachine = (AIGoblinStateMachine)stateMachine;
	...
	// -------------------------------------------------------------------------------------
	// Name	:	OnTriggerEvent
	// Desc	:	Called by the parent state machine when threats enter/stay/exit the goblin's
	...
	public override void OnTriggerEvent(AITriggerEventType eventType, Collider other)
	{
		...
		if (eventType != AITriggerEventType.Exit)
		{
			...
			AITargetType curType = _goblinStateMachine.VisualThreat.type;

			// Is the collider that has entered our sensor a player
			if (other.CompareTag("Player"))
			{
				// Get distance from the sensor origin to the collider
				float distance = Vector3.Distance(_goblinStateMachine.sensorPosition, other.transform.position);
				...
				if (curType != AITargetType.Visual_Player ||
					(curType == AITargetType.Visual_Player && distance < _goblinStateMachine.VisualThreat.distance))
				{
					...
					if (ColliderIsVisible(other, out hitInfo, _playerLayerMask))
					{
						...
						_goblinStateMachine.VisualThreat.Set(AITargetType.Visual_Player, other, other.transform.position, distance);
					}
				}
			}
			else
			if (other.CompareTag("AI Sound Emitter"))
			{
				...
				Vector3 agentSensorPosition = _goblinStateMachine.sensorPosition;
				...
				distanceFactor += distanceFactor * (1.0f - _goblinStateMachine.hearing);
				...
				if (distanceToThreat < _goblinStateMachine.AudioThreat.distance)
				{
					// Most dangerous Audio Threat so far
					_goblinStateMachine.AudioThreat.Set(AITargetType.Audio, other, soundPos, distanceToThreat);
				}
			}
		}
	}
	...
	// Desc	:	Test the passed collider against the goblin's FOV and using the passed
	...
	protected virtual bool ColliderIsVisible(Collider other, out RaycastHit hitInfo, int layerMask = -1)
	{
		...
		// We need the state machine to be an AIGoblinStateMachine
		if (_goblinStateMachine == null) return false;
		...
		if (angle > _goblinStateMachine.fov * 0.5f)
			return false;
		...
		// of our sensor radius scaled by the goblin's sight ability. This will return ALL hits.
		RaycastHit[] hits = Physics.RaycastAll(head, direction.normalized, _goblinStateMachine.sensorRadius * _goblinStateMachine.sight, layerMask);
	...
	public void BatCollisionEnable()
	{
		_batCollider.enabled = true;
	}

	public void BatColliderDisable()
	{
		_batCollider.enabled = false;
	}

	public void ShieldCollisionEnable()
	{
